...
# Code borrowed from https://github.com/edwardlib/observations/blob/master/observations/util.py
...
class SNELSON:
    ...
    def load_snelson(self):
        X,Y = snelson1d(self.cfg.data.data_path)
        ...
        self.train_loader = DataLoader(self.train_dataset, batch_size=self.cfg.data.batch_size_train, shuffle=True)
        
        self.test_dataset = TensorDataset(torch.tensor(X_test, dtype=torch.float32), torch.tensor(Y_test, dtype=torch.float32))
        self.test_loader = DataLoader(self.test_dataset, batch_size=self.cfg.data.batch_size_test, shuffle=False)
    ...
    def create_inference_dataloader(self):
        ...
        test_loader = DataLoader(test_dataset, batch_size=self.cfg.data.batch_size_test, shuffle=False)

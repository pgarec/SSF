...
from model_merging.merging import merging_models_ensemble
...
def evaluate(cfg, merged_model, models, criterion, test_loader):
    ...
    for m in range(len(models)):
        ...
        plt.bar(np.arange(len(cfg.data.digits)), avg_loss)
        plt.xlabel("Number of classes model {}".format(m))
...
def main(cfg):
    ...
    merged_model = merging_models_ensemble(cfg, models, fishers)

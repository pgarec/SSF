    ...
    def forward(self):
        ...
        for a in range(args.dim):
            ...
            for p in range(args.max_perm):
                # permutations where here before -- now they changed position
                model_0 = models[0]
                for k, model_k in enumerate(models):
                    ...
                    S_k = model_k['S']
                    if k==0:
                        theta_prev = model_0['m']
                        theta_r = theta_prev[m:]
                    else:
                        theta_prev = model_0['m']
                        theta_r = theta_prev[m:]
                    ...
                    # log_p_masked = -0.5*torch.log(v_pred) - 0.5*np.log(2*np.pi) - (0.5*(theta_prev[:m] - m_pred)**2 / v_pred)
                    log_p_masked = -0.5*torch.log(v_pred) - 0.5*np.log(2*np.pi) - (0.5*(theta[:m] - m_pred)**2 / v_pred)
                    loss_pred += log_p_masked.sum(1)
                    model_0 = model_k

            loss_pred = loss_pred/(args.max_perm * m)
            loss += loss_pred.sum()

...
if __name__ == "__main__":
    ...
    plt.plot(torch.arange(len(loss_mnist1[:3])), [0.09114535762985346]*(len(loss_mnist1[:3])),'--', lw=2.0, color=palette_green[3], alpha=0.9, label="Isotropic")
    plt.plot(torch.arange(len(loss_mnist1[:3])), [0.06974360538691057]*(len(loss_mnist1[:3])),'--', lw=2.0, color=palette_red[0], alpha=0.9, label="Fisher")
    plt.plot(loss_mnist1[:3], lw=3, alpha=0.6, color=palette_red[3+1], label="SSF")
    plt.plot(loss_mnist2[:3], lw=3, alpha=0.6, color=palette_red[3+2])
    plt.plot(loss_mnist3[:3], lw=3, alpha=0.6, color=palette_red[3+3])
    # plt.plot(loss_mnist4, lw=4.5, alpha=0.4, color=palette_blue[3+4])

    plt.ylim(0,max(loss_mnist2[:3]))
    num_points = len(loss_mnist2[:3])
